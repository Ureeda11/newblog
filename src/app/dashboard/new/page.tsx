...
import { ImageIcon, X, Tag, Save, Globe, Loader2 } from 'lucide-react'
...
async function handleSave(publishStatus: 'draft' | 'published') {
  if (!title || !content || !excerpt) return alert('Title, excerpt and content are required')
  setSaving(true)
  try {
    const res = await fetch('/api/posts', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title, content, excerpt, tags, coverImage, status: publishStatus }),
    })
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <h1 style={{ fontSize: '1.6rem', fontWeight: 900 }}>New Post</h1>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={() => handleSave('published')} disabled={saving} style={{
            ...
            borderRadius: 10, cursor: 'pointer', fontSize: '0.875rem', fontWeight: 500,
          }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
        ...
        <textarea
          ...
          style={{
            ...
            background: 'var(--surface)', border: '1px solid var(--border)',
            borderRadius: 12, color: 'var(--text)', fontSize: '0.95rem',
        ...
        {/* AI Editor */}
        <AIEditor content={content} onChange={setContent} />
